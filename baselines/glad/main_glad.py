...
from baselines.glad import glad
from baselines.glad.glad_model import glad_model
...
parser.add_argument('--MODEL_SELECT', type=str,  default='Fb', 
                    help='select model based on best results: {graph recovery}Fb=auc/shd/current ')
...
def glad_train_batch(train_data, valid_data=None):
    print('Training phase of glad: batch')
...
#    for i, d in enumerate(train_data):
    for i, ri in enumerate(rd):
        ...
        X_centered = convert_to_torch(X_centered)    
        ...
        S = torch.matmul(X_centered.t(), X_centered)/X_centered.shape[0] # DxD matrix
        for epoch in range(args.EPOCHS):
            ...
            if epoch % args.PRINT_EPOCH == 0:
                print('\nBatch = ',batch_num, ' epoch = ', epoch, ' loss glad = ', loss_glad.detach().cpu().numpy()[0])
                print('Recovery :FDR, TPR, FPR, SHD, nnz_true, nnz_pred, precision, recall, Fb, aupr, auc ', *np.around(compare_theta(theta, theta_s), 3))
            t2 = time.time()
            loss_glad.backward() # calculate the gradients
            optimizer_glad.step() # update the weights
            t3 = time.time()
            # Choose the best model based on validation result
            if len(valid_data)>0 and (epoch%50==0 or epoch==args.EPOCHS-1):# and args.MODEL_SELECT!='current':
                # best best Fb model
                ...
                print('time req for glad forward call (secs): ', t2-t1)
...
def f_beta_loss(out, target, beta=args.beta):
...
def mse_f_beta(masked_out, masked_target):
...
def glad_predict_single(model, model_auxiliary, theta, X_centered, PRINT=True, pair_num=-1, tf_names=[]):
    model_glad = model
    criterion_graph =  model_auxiliary
    # preparing the data
    X_centered = convert_to_torch(X_centered, TESTING_FLAG=True) 
    theta_true = convert_to_torch(theta, TESTING_FLAG=True)
    ...
    if args.USE_TF_NAMES=='yes' and len(tf_names) != 0:
        mask_tf = get_tf_mask(tf_names, dim=S.shape[-1])
