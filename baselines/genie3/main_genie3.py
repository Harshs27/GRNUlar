...
from numpy import *
#from simulator.sim_expt3.data.gen_data import gen_data
...
def fit_genie3(data, PREDICT_TF=False):
    EARLY_BREAK = 9 
    print('FITTING GENIE3')
    # #############################################################################
    res = []
    typeS = 'mean'
    ...
    for i, d in enumerate(data):
        ...
        print('\n genie3: TRAIN data batch : ', i, ' total points = ', X.shape[0])
        if args.USE_TF_NAMES=='yes' and PREDICT_TF:
            res.append(helper_genie3(Xc, theta_true, tf_names = master_regulators))
        else:
            # NOTE: breaking early as tf=None takes lot of time
            if i > EARLY_BREAK:
                print('Breaking at i = ', i, ' as tf=None case takes a lot of time')
                break
            res.append(helper_genie3(Xc, theta_true))
    ...
    if PREDICT_TF:
        print('\nAvg GENIE3-TF: FDR, ,TPR, ,FPR, ,SHD, ,nnz_true, ,nnz_pred, ,precision, ,recall, ,Fb, ,aupr, ,auc, ')
    else:
        print('\nAvg GENIE3: FDR, ,TPR, ,FPR, ,SHD, ,nnz_true, ,nnz_pred, ,precision, ,recall, ,Fb, ,aupr, ,auc, ')
...
def helper_genie3(X, theta_true, tf_names=[], BEELINE=False):#_string
    ...
    network = genie3(expression_data=ex_matrix, gene_names=gene_names, tf_names=tf_names)#, verbose=True)
...
#    pred_theta = nx.adj_matrix(G_pred).todense() + np.eye(args.D)
    ...
    print('GENIE3: FDR, TPR, FPR, SHD, nnz_true, nnz_pred, precision, recall, Fb, aupr, auc')
...
def load_BEELINE_data():
    print('Loading the processed BEELINE real data')
    FILEPATH = 'simulator/BEELINE-data/beeline_processed.pickle'
    with open(FILEPATH, 'rb') as handle:
        data = pickle.load(handle)
    return data
...
#        valid_data = gen_data.add_technical_noise(args, valid_data)
    ...
    # Fitting a genie3
    fit_genie3(test_data)
    print('Using TF NAMES')
    fit_genie3(test_data, PREDICT_TF=True)
    print('\nExpt Done')
    return 
